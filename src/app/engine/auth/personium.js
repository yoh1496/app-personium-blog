...
exports.personium = (function() {
  ...
  personium.getAppToken = function(p_target) {
    var ret;
    var appCell = _p.as('serviceSubject').cell(p_target);
  ...
  personium.getProtectedBoxAccessToken4ExtCell = function(
    cellUrl,
    transcellToken,
    appToken
  ) {
  ...
  personium.verifyOrigin = function(request) {
    ...
    for (i = 0; i < refererUrlList.length; i++) {
      if (refererUrl && refererUrl.indexOf(refererUrlList[i]) == 0) {
        urlAllowed = true;
        matchedOrigin = refererUrlList[i];
        break;
      }
    }
    if (!urlAllowed) {
  ...
  personium.validateKeys = function(params) {
    ...
    if (_.isEmpty(invalidParams)) {
      ...
      var hasRequiredInfo = _.every(_requiredKeys, function(
        element,
        index,
        list
      ) {
        return (
          _.has(params, element) &&
          !_.isEmpty(params[element]) &&
  ...
  personium.httpPOSTMethod = function(
    url,
    headers,
    contentType,
    body,
  ...
  personium.createErrorResponse = function(e) {
    ...
    if (
      _.isUndefined(tempErrorCode) ||
      _.isNull(tempErrorCode) ||
      tempErrorCode == 0
    ) {
